...
if ($nv_Request->isset_request('contentid', 'get,post') and $fcheckss == $checkss) {
    ...
    if ($nv_Request->isset_request('contentid', 'post')) {
        ...
        if (file_exists(NV_ROOTDIR.'/uploads/news/audio/'.$rowcontent['audio'])) {    
            $rowcontent['audio'] = $rowcontent['audio'];           
        }else {
            $rowcontent['audio'] = '';
        }
        ...
        elseif (isset($fcode) and !nv_capcha_txt($fcode, $module_captcha)) {
            ...
            if ($rowcontent['id'] == 0) {
                ...
                $_sql = 'INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '_rows
                        (catid, listcatid, topicid, admin_id, author, sourceid, addtime, edittime, status, weight, publtime, exptime, archive, title, alias, audio, hometext, homeimgfile, homeimgalt, homeimgthumb, inhome, allowed_comm, allowed_rating, external_link, hitstotal, hitscm, total_rating, click_rating) VALUES
                ...
                $_sql = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_rows SET
                         ...
                         audio=' . $db->quote($rowcontent['audio']) . ',
