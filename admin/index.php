...
if(isset($_GET['action']) && $_GET['action'] == "upload_video"){   
  
    $results = array();
    $results['data'] = '';
    $allowedExts = array("jpg", "jpeg", "gif", "png", "mp3", "mp4", "wma", "mov");
    $extension = pathinfo($_FILES['file']['name'], PATHINFO_EXTENSION);
    $path = NV_ROOTDIR.'/uploads/news/audio/';
    $_FILES["file"]["name"] = str_replace(" ", "", $_FILES["file"]["name"]);
    //echo $path;exit;
    //if (($_FILES["file"]["type"] == "video/mp4") || ($_FILES["file"]["type"] == "audio/mp3") || ($_FILES["file"]["type"] == "audio/wma") && ($_FILES["file"]["size"] < 20000) && in_array($extension, $allowedExts)){
      //echo "<pre>";print_r($_FILES);echo "</pre>";exit;
      if ($_FILES["file"]["error"] > 0){
            $results['status'] = 0;
            $results['msg'] = "Upload file error!";
            
      }else{
        
          if (file_exists($path . $_FILES["file"]["name"])){
              $file_name = str_replace(".".$extension, "", $_FILES["file"]["name"])."_".time().".".$extension;
          ...
          if(file_exists($path . $file_name)){
              $results['status'] = 1;
              $results['msg'] = "File upload success";
