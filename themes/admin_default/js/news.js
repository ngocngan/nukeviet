...
$(document).ready(function() {
    ...
    $("#select-video-post").click(function() {
        $('#file_video_upload').click();
    });
    
    $("#file_video_upload").change(function() {       
    ...
    $('#form_upload_video' ).submit(
            function( e ) {
                $.ajax( {
                    url: 'index.php?action=upload_video',
                    type: 'POST',
                    ...
                    success: function(result){
                        ...
                        if(json_data['status'] == 1){
                            $('#audio_path').val(json_data['data']);
                            $('#select-video-post').after('<p class="rp-success">'+json_data['msg']+'</p>')
                        }else{
                            $('#select-video-post').after('<p class="rp-error">'+json_data['msg']+'</p>')
                        }
                        setTimeout(function(){
                            $('.rp-error').remove(); 
                            $('.rp-success').remove(); 
